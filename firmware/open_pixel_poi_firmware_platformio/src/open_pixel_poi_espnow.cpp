...
public:
  ...
  void setup() {
    // 1. Wi-Fi Station Mode without promiscuous lock
    WiFi.mode(WIFI_STA);
    WiFi.disconnect();
    ...
    // 2. Dynamic channel 0: follows active radio automatically (coexists cleanly with BLE!)
  ...
  void handleIncomingPacket(const PoiSyncPacket* pkt) {
    ...
    if (pkt->senderId == myNodeId) return; // Ignore own echo

    bool bankChanged = (config.patternBank != pkt->patternBank);
    bool slotChanged = (config.patternSlot != pkt->patternSlot);

    if (bankChanged) {
      config.setPatternBank(pkt->patternBank, false);
    }
    if (slotChanged || bankChanged) {
      config.setPatternSlot(pkt->patternSlot, false);
    }
